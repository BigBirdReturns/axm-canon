...
describe("AGOT repository patch worktree executor exact carrier", () => {
  it("reconstructs the exact source-rebuilt ZIP from ordered chunks", () => {
    ...
    expect(carrier.base64Characters).toBe(21644);
    expect(carrier.concatenationOrder).toEqual(["carrier/part-0001.b64", "carrier/part-0002.b64", "carrier/part-0003.b64", "carrier/part-0004.b64", "carrier/part-0005.b64"]);
  ...
  it("extracts safely and replays all static and synthetic checks", () => {
    ...
    try {
      ...
      expect(JSON.parse(campaign.stdout)).toMatchObject({ passed: 17, total: 17, status: "PASS", fixtureSourceTextOnly: true, realPatchBundlesConsumed: 0, realWorktreePatchesApplied: 0, gitIndexMutations: 0, gitCommitsCreated: 0, localReferenceUpdates: 0, remotePushes: 0 });
    } finally { rmSync(tempRoot, { recursive: true, force: true }); }
  });

  it("retains the exact upstream and no-effect authority boundaries", () => {
    ...
    expect(admission.authorityBoundary).toMatchObject({ privateSourceTextPresent: false, privatePayloadPresent: false, worktreeApplyIsNotCommit: true, commitIsNotReferenceUpdate: true, referenceUpdateIsNotPush: true, repositoryMutationExecuted: false, canonEffect: "none", graphEffect: "none" });
  });
});
