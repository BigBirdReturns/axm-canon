...
A separately reconstructed So Spake Martin successor now exists at [`../2026-08-19-ssm-bounded-proposition-registry-v1/`](../2026-08-19-ssm-bounded-proposition-registry-v1/README.md). It descends ten live public routes into twenty-three bounded paraphrases under a new component identity, preserves named submitters and reporter mediation, and retains the exact v0.1 SSM archive as an unmaterialized parent. It is not an exact-file admission of the `ssm-item-descent` component recorded in `ADMISSION.json`, so that component also remains `payloadMaterialized: false`.
