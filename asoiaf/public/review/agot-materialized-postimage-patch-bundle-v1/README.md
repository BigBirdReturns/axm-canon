...
The static verifier passes 174 of 174 checks. The noncopyrighted adversarial campaign passes 30 of 30 cases. A separate compatibility test passes the emitted bundle through the actual admitted worktree executor and proves that only the two governed worktree paths change while HEAD, the feature reference, and the live index remain unchanged.

Runtime success stops at `PASS_MATERIALIZED_POSTIMAGE_PATCH_BUNDLE_SEALED_WORKTREE_APPLICATION_WITHHELD` with `LOCAL_FEATURE_WORKTREE_APPLICATION_WITHHELD`. Carrier reconstruction and repository qualification do not consume a real receipt, seal a real patch bundle, apply a worktree change, create a commit, update a reference, push a remote, open a pull request, promote canon, or mutate the graph.
