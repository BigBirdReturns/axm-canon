...
const repair = JSON.parse(readFileSync(resolve(root, "REPAIR.json"), "utf8"));

function sha256(data: Buffer | string): string {
  return createHash("sha256").update(data).digest("hex");
}
function gitBlobSha1(data: Buffer): string {
  return createHash("sha1").update(Buffer.from(`blob ${data.length}\0`, "utf8")).update(data).digest("hex");
}
...
with zipfile.ZipFile(archive) as handle:
    ...
    for info in handle.infolist():
        name=info.filename.replace("\\", "/"); path=pathlib.PurePosixPath(name); mode=info.external_attr >> 16
        if path.is_absolute() or ".." in path.parts or stat.S_ISLNK(mode): raise SystemExit(f"unsafe zip member: {name}")
        names.append(name)
    handle.extractall(destination)
print(json.dumps({"members":len(names)}))
`;

describe("AGOT exact physical-recovery handoff carrier repair", () => {
  it("reconstructs the exact Windows ZIP from all nineteen reachable chunks", () => {
    expect(carrier.chunkCount).toBe(19);
    expect(carrier.chunks).toHaveLength(19);
    ...
    expect(repair.preRepair.reconstructable).toBe(false);
    expect(repair.repair.identityChanged).toBe(false);
  ...
  it("extracts safely and replays the complete 27-check handoff verifier", () => {
    ...
    try {
      const archivePath = join(tempRoot, admission.package.filename); writeFileSync(archivePath, archive);
      const extraction = spawnSync(python(), ["-c", safeExtract, archivePath, tempRoot], { encoding: "utf8" });
      ...
      expect(JSON.parse(replay.stdout)).toMatchObject({
        componentId: "asoiaf-agot-recovery-to-review-handoff-v1", passed: 27, total: 27, status: "PASS",
        warningsAsErrors: true, reviewTransactionsExecuted: 0, automaticCanonPromotions: 0,
        automaticGraphMutations: 0, copyrightedSourceTextUsed: false, sourceTextReturned: false,
      });
    } finally { rmSync(tempRoot, { recursive: true, force: true }); }
  });

  it("preserves private-source and review authority boundaries", () => {
    expect(admission.counts).toEqual({
      recoveryTargets: 4, atomicReviewCandidates: 45, sourceParents: 31, privateParagraphLocators: 31,
      expandedFiles: 45, reviewTransactionsExecuted: 0, automaticCanonPromotions: 0, automaticGraphMutations: 0,
    });
    ...
    expect(admission.authorityBoundary).toMatchObject({
      assistantCannotServeAsNamedHumanReviewer: true, carrierDoesNotImpersonateArchivePath: true,
      exactRecoveryDoesNotEqualHumanReview: true, locatorResolutionDoesNotAuthorizePrivateDisplay: true,
      sourceArchiveRecoveryDoesNotAuthorizeExtraction: true, privatePayloadPresent: false, sourceTextPresent: false,
      automaticCanonEffect: "none", automaticGraphEffect: "none",
    });
  });
});
